...
senparc.menu = {
    ...
    init: function () {
        ...
        $('#btnResetAccessToken').click(function () {
            menuState.html('正在获取...');
            //$.getJSON('/WeixinMenuTool/GetToken?t=' + Math.random(), { appId: $('#menuLogin_AppId').val(), appSecret: $('#menuLogin_AppSecret').val() },
            $.getJSON('/WeixinMenuTool/GetToken?t=' + Math.random(), { },
                function (json) {
                    if (json.access_token) {
                        senparc.menu.setToken(json.access_token);
                        menuState.html('获取成功');
                    } else {
                        menuState.html('获取错误：'+ json.Data);
                    }
                });
            return false;
        });
        ...
        $('#btnGetMenu').click(function () {
            ...
            $.getJSON('/WeixinMenuTool/GetMenu?t=' + Math.random(), { token: senparc.menu.token }, function (json) {
                if (json.menu) {
        ...
        $('#btnDeleteMenu').click(function () {
            ...
            $.getJSON('/WeixinMenuTool/DeleteMenu?t=' + Math.random(), { token: senparc.menu.token }, function (json) {
                if (json.Success) {
        ...
        $('#submitJsonMenu').click(function () {
            ...
            $.post('/WeixinMenuTool/CreateMenuFromJson', { token: $('#tokenStr').val(), fullJson: jsonStr }, function (json) {
                if (json.Success) {
                    menuState.html('上传成功');
                } else {
                    menuState.html(json.Message + "：" + json.Data);
                }
            }, "json");
        });
