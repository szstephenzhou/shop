...
/* 验证日期格式：yyyy-MM-dd */
laydate.checkDate = function (cls) {
    var reg = /^((((1[6-9]|[2-9]\d)\d{2})-(0?[13578]|1[02])-(0?[1-9]|[12]\d|3[01]))|(((1[6-9]|[2-9]\d)\d{2})-(0?[13456789]|1[012])-(0?[1-9]|[12]\d|30))|(((1[6-9]|[2-9]\d)\d{2})-0?2-(0?[1-9]|1\d|2[0-8]))|(((1[6-9]|[2-9]\d)(0[48]|[2468][048]|[13579][26])|((16|[2468][048]|[3579][26])00))-0?2-29-))$/;
    var exp = new RegExp(reg), val = '', rst = true;
    $('.' + cls).each(function () {
        val = $.trim(this.value);
        if ($.trim(val) !== '' && !exp.test(val)) { //不符合日期格式
            $(this).click();
            rst = false;
            return false;
        }
...
/* 验证日期格式(包括空值)：yyyy-MM-dd */
laydate.checkNullDate = function (cls) {
    var reg = /^((((1[6-9]|[2-9]\d)\d{2})-(0?[13578]|1[02])-(0?[1-9]|[12]\d|3[01]))|(((1[6-9]|[2-9]\d)\d{2})-(0?[13456789]|1[012])-(0?[1-9]|[12]\d|30))|(((1[6-9]|[2-9]\d)\d{2})-0?2-(0?[1-9]|1\d|2[0-8]))|(((1[6-9]|[2-9]\d)(0[48]|[2468][048]|[13579][26])|((16|[2468][048]|[3579][26])00))-0?2-29-))$/;
    var exp = new RegExp(reg), val = '', rst = true;
    $('.' + cls).each(function () {
        val = $.trim(this.value);
...
/* 验证时间格式：hh:mm:ss */
laydate.checkTime = function (cls) {
    var rst = true;
    var reg = /^(20|21|22|23|[0-1]\d):[0-5]\d:[0-5]\d$/;
    var exp = new RegExp(reg), val = '', rst = true;
    $('.' + cls).each(function () {
        val = $.trim(this.value);
        if ($.trim(val) !== '' && !exp.test(val)) { //不符合时间格式
            $(this).click();
            rst = false;
            return false;
        }
...
/* 验证日期+时间格式：yyyy-MM-dd hh:mm:ss */
laydate.checkDateTime = function (cls) {
    ...
    $('.' + cls).each(function () {
        val = $.trim(this.value);
        if ($.trim(val) !== '' && !exp.test(val)) { //不符合日期+时间格式
            $(this).click();
            rst = false;
            return false;
        }
