...
(function ($) {	
	...
	function require(options){
		var rule={//验证规则
		    phone: /^(((13[0-9]{1})|(14[579])|(15[0-9]{1})|(17[0-9]{1})|(18[0-9]{1}))+\d{8})$/,
			company:/^[\u4E00-\u9FA5a-zA-Z][\u4E00-\u9FA5a-zA-Z0-9\s-,-.]*$/,
			uname:/^[\u4E00-\u9FA5a-zA-Z][\u4E00-\u9FA5a-zA-Z0-9_]*$/,
			zh:/^[\u4e00-\u9fa5]+$/,//纯中文
			en: /^[A-Za-z]+$/,//纯英文
			numen: /^[A-Z a-z 0-9]+$/,//纯数字+英文
			...
			int: /^[0-9]*$/,
			float: /^[0-9]+\.?[0-9]*$/,
			carno: /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼领A-Z0-9]{1}[A-Z0-9]{1}[A-Z0-9]{4,5}[A-Z_0-9挂学警港澳使]{1}$/,
			pwd: /^[A-Z a-z 0-9 ~!@#$%^&*()_+|<>?]+$/,//数字+英文+英文符号
			s:''
		};
		this.rules= {
	        ...
	        isFloat: function (value, errorMsg) {
	            errorMsg = errorMsg || " ";
	            if (!rule.float.test(value)) return errorMsg;
	        },
	        ...
	        isEn: function(value, errorMsg) {
	            errorMsg = errorMsg || " ";
	            if (!rule.en.test(value)) return errorMsg;
	        },
		    isNumEn: function(value, errorMsg) {
		        errorMsg = errorMsg || " ";
		        if (!rule.numen.test(value)) return errorMsg;
		    },
	        ...
	        isCarNo: function (value, errorMsg) {
	            errorMsg = errorMsg || " "; 
	            if (value.search(rule.carno) === -1) {
                    return errorMsg;
	            }
	        },
            isPwd:function(value, errorMsg) {
                errorMsg = errorMsg || " ";
                if (value.search(rule.pwd) === -1) {
                    return errorMsg;
                }
            }
	    }
...
$(function(){
    ...
    //限制只能输入金额
	$('body').on("keypress", ".money", function () {
        if (event.keyCode !== 46 && (event.keyCode < 48 || event.keyCode > 57)) {
            event.returnValue = false;
        }
    });
    //限制只能输入0-9数字
	$('body').on("keypress", ".int", function () {
